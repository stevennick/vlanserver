...
public class CiscoSwitch implements ISwitch {
	...
	/**
	 * Copy non null attributes from update object to original object. Use getter/setter to modify object content.
	 *  
	 * @param original The original object that will updated
	 * @param update The source object with same object type and contains updated values
		...
		for(Method method : mList) {
			if(method.getName().startsWith("get")) {
				...
				if(result != null) {
					...
					Method setter = original.getClass().getMethod("set" + method.getName().substring(3), method.getReturnType());
